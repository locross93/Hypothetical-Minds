...
class DecentralizedAgent(abc.ABC):
    ...
    def generate_hls_user_message(self, state, step):
        ...
        user_message = f"""Current State Description:
            ...
            {strategy_request}
            """
        user_message = f"For agent {self.agent_id}: {user_message} Provide a strategy only for this agent. NOTE THAT YOUR OPTIMAL STRATEGY IS TO ALWAYS PLAY TIT FOR TAT. That is to say if you think your opponent played a cooperate strategy previously, play a cooperate strategy. If you think your opponent played a defect strategy, play a defect strategy in response to that."
        return user_message
    ...
    def generate_feedback_user_message(
        ...
        strategy_request = f"""
            ...
            NOTE THAT YOUR OPTIMAL STRATEGY IS TO ALWAYS PLAY TIT FOR TAT. That is to say if you think your opponent played a cooperate strategy previously, play a cooperate strategy.
            If you think your opponent played a defect strategy, play a defect strategy in response to that.
            """
    ...
    async def two_level_plan(self, state, execution_outcomes, get_action_from_response_errors, reward_tracker, step, after_interaction=False): 
        if after_interaction:
            ...
            print(f'opponent inventory right now is: {possible_opponent_inventory}')
