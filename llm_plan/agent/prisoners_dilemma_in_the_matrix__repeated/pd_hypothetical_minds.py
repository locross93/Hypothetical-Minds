...
class DecentralizedAgent(abc.ABC):
    ...
    def generate_hls_user_message(self, state, step):
        ...
        user_message = f"""Current State Description:
            ...
            - Observable Other Player Locations: {opponent_locations}
            - Previously seen states from memory (format: ((x,y), step last observed, distance from current location): {self.memory_states}
            
            {strategy_request}
            """
        user_message = f"For agent {self.agent_id}: {user_message} Provide a strategy only for this agent. Your goal is to maximize reward attained over an entire episode, so keep in mind the long-term consequences of your actions. Look at events in a gestalt manner."
        return user_message
    ...
    def generate_feedback_user_message(
        ...
        strategy_request = f"""
            Strategy Request:
            You are at step {step} of the game.
            You have decided to execute a high-level strategy/target inventory in a previous response given what you predicted the other player will do.
            Select subgoals in order to achieve the target inventory, denoted by 'my_next_inventory' in this dictionary: {self.my_next_inventory}.
            ...
            After collecting these resources, I will look for player_1 to initiate a duel.
            ```python
            {{
            ...
            targeting an inventory of 5 green and 1 red. Once I have collected these resources, I will attempt to find and duel with player_1.
            ```python
            {{
            ...
            - fire_at(target_coord): Stay around specified coordinate and fire interaction when player_1 is spotted to initiate duel. 
    ...
    def generate_interaction_feedback_user_message1(
        ...
        user_message = f"""
            ...
            What was the other player's likely inventory in the last round given the inventory I played and the reward received?
            To deduce the other player's inventory, consider the balance of resources in your inventory and the nature of the reward. 
            Reflect on the examples provided to determine how to infer the possible other player inventory. Notice patterns in how the rewards vary 
            ...
            - if you played a strategy of cooperation:
                - and received a low reward (<1), then the other player likely played a strategy of strong defection,
                - and received a medium reward (>2.8 and <3.66), then the other player likely played a strategy of strong cooperation,
            - if you played a strategy of defection, 
                - and received a high reward (>3.8), then the other player likely played a strategy of strong cooperation,
                - and received a medium reward (>1 and <1.5), then the other player likely played a strategy of strong defection,
            ...
            'your_inventory': {{'cooperate/green': 6, 'defect/red': 1}}, 'rewards': 0.59, 'possible_other_player_inventory': {{'cooperate/green': 1, 'defect/red': 6}}
            'your_inventory': {{'cooperate/green': 1, 'defect/red': 6}}, 'rewards': 4.16, 'possible_other_player_inventory': {{'cooperate/green': 6, 'defect/red': 1}}
            'your_inventory': {{'cooperate/green': 5, 'defect/red': 1}}, 'rewards': 2.80, 'possible_other_player_inventory': {{'cooperate/green': 5, 'defect/red': 1}}
            'your_inventory': {{'cooperate/green': 1, 'defect/red': 7}}, 'rewards': 1.31, 'possible_other_player_inventory': {{'cooperate/green': 1, 'defect/red': 8}}
            'your_inventory': {{'cooperate/green': 4, 'defect/red': 1}}, 'rewards': 2.60, 'possible_other_player_inventory': {{'cooperate/green': 3, 'defect/red': 1}}
            In the 2nd part of your response, output the predicted other player's inventory in following Python dictionary format, parsable by `ast.literal_eval()` starting with ```python.
            Example output:
            Given that I last played a strong cooperate strategy with an inventory of {{'cooperate/green': 6, 'defect/red': 1}} and received a small reward of 0.59, I believe my player 1 played a strong defect strategy. 
            ...
            {{
              'possible_other_player_inventory': {{'cooperate/green': 1, 'defect/red': 6}}
            }}
            ```
            Your goal is to maximize reward attained over an entire episode, so keep in mind the long-term consequences of your actions. Look at events in a gestalt manner.
            """
    ...
    def generate_interaction_feedback_user_message2(
        ...
        if self.self_improve:
            strategy_request = f"""
                ...
                Here are your previous hypotheses about the algorithm your player_1 is playing: {self.top_hypotheses}.
                What is your player_1's likely policy given the inventories and the reward?  Remember the rule for how the rewards vary is based on 
                ...
                - if you played a strategy of cooperation:
                    - and received a low reward (<1), then the player_1 likely played a strategy of strong defection,
                    - and received a medium reward (>2.8 and <3.66), then the player_1likely played a strategy of strong cooperation,
                - if you played a strategy of defection, 
                    - and received a high reward (>3.8), then the player_1 likely played a strategy of strong cooperation,
                    - and received a medium reward (>1 and <1.5), then the player_1 likely played a strategy of strong defection,
                ...
                If so, your player_1 may be playing a pure strategy (like always cooperating) and you can exploit this (by always defecting if they are always cooperating).
                Once you have output a hypothesis about your player_1's strategy with step by step reaasoning, you can use hypothesis to inform your strategy. 
                ...
                ```python
                {{
                'Other_player_strategy': 'I think my player_1 is always playing a pure cooperate strategy and collecting around 5 green resources.'
                }}
                ```
            ...
            strategy_request = f"""
                An interaction with the other player has occurred at step {step}, {self.interaction_history[-1]}.
                The total interaction history is: {self.interaction_history}.
                What is your player_1's likely policy given the inventories and the reward function? Think step by step about this given the interaction history. 
                They may be playing the same pure policy every time, a complex strategy to adapt to you, or anything in between. 
                They are not necessarily a smart agent that adapts to your strategy, you are just playing an algorithm. 
                Are you getting high or low reward when playing the same type of inventory? 
                For example getting high reward every time you play many defect/red resources. 
    ...
    def generate_interaction_feedback_user_message3(
        ...
        user_message = f"""
            ...
            Given this strategy and the interaction history, predict what the other player will play next (player 1).
            
            Please remember the nuance of 'prisoner_dilemma_in_the_matrix__repeated' - the rewards are using the payoff rules of the classic prisoner dilemma, but the interaction is repeated across many interactions in an episode.
            Also the rewards are distributed based on the inventory of resources collected by each player.          

            This response should include step by step reasoning in the following format:
            1. 'Other_player_next_inventory': Given the above mentioned guess about the player_1's policy/strategy, and the last inventory you played (if their strategy is adaptive, it may not be), what is their likely inventory in the next round.
            2. In the 2nd part of your response, output the predicted player_1's next inventory in following Python dictionary format, parsable by `ast.literal_eval()` starting with ```python.
            Example response:
            1. 'Other_player_next_inventory': Given that player_1 is playing a cooperate policy, I believe their next inventory will be inventory={{'cooperate/green': 5, 'defect/red': 1}}.
            ```python
            {{
              'predicted_other_player_next_inventory': {{'cooperate/green': 5, 'defect/red': 1}}
            }}
            """
        print(user_message)
        return user_message
    ...
    def generate_interaction_feedback_user_message4(
        ...
        user_message = f"""
            ...
            You last played: {self.interaction_history[-1]['your_inventory']}
            You previously guessed that their policy or strategy is: {possible_opponent_strategy}.
            You previously guesssed that their next inventory will be: {self.possible_opponent_inventory}.
            High-level strategy Request:            
            Given the above information, provide the next high-level strategy for player {self.agent_id} to maximize long term reward.
            
            Please remember the nuance of 'prisoner_dilemma_in_the_matrix__repeated' - the rewards are using the payoff rules of the classic prisoner dilemma, but the interaction is repeated across many interactions in an episode.
            Also the rewards are distributed based on the inventory of resources collected by each player.           

            This response should include step by step reasoning in parts 1, 2, and 3 about which long term strategy to select based on the entire interaction history and other player's strategy in the following format:
            1. My strategy: what should your long term strategy be? If you're playing with an adaptive player, think about what actions will lead them to play inventories that are better for you in the long term. 
            Remember this is mixed-motive environment, not a competitive one. For example, what is the best strategy to play against an adaptive player to ensure that they cooperate consistently over multiple rounds? 
            2. My next inventory: Given the player_1's likely inventory in the next round, and your long term strategy, select a next inventory to play based on this. 
            3. In the 3rd part of your response, output your next inventory in following Python dictionary format, parsable by ast.literal_eval() starting with python.
            Example response:
            1. My strategy: Given that my player_1 is always playing a cooperate policy, I believe my long term strategy should be to defect.
            2. My next inventory: My next inventory to maximize reward should be a defect policy inventory={{'cooperate/green': 1, 'defect/red': 5}}.
            ```python
            {{
    ...
    async def two_level_plan(self, state, execution_outcomes, get_action_from_response_errors, reward_tracker, step, after_interaction=False): 
        if after_interaction:
            ...
            while not correct_syntax and counter < 6:
                ...
                #at this point, I want to measure possible_opponent_inventory to actual opponent inventory
                key_present = 'possible_other_player_inventory' in possible_opponent_inventory
                
                if not key_present:
                    correct_syntax = False
                    print(f"Error parsing dictionary when extracting other player inventory, retrying...")
                counter += 1
            self.possible_opponent_inventory = possible_opponent_inventory
            print(f'other player inventory right now is: {possible_opponent_inventory}')
            ...
            if not self.good_hypothesis_found:
                ...
                while not correct_syntax and counter < 6:
                    ...
                    for i in range(len(responses)):
                        ...
                        other_player_next_inventory = self.extract_dict(response)
                        if 'predicted_other_player_next_inventory' not in other_player_next_inventory:
                            correct_syntax = False
                            print(f"Error parsing dictionary when extracting next inventories, retrying...")
                            break
                        if i == 0:
                            self.other_player_next_inventory = deepcopy(other_player_next_inventory)
                            self.opponent_hypotheses[self.interaction_num]['other_player_next_inventory'] = deepcopy(self.other_player_next_inventory)
                            hls_response = hls_response + '\n\n' + response
                        else:
                            self.opponent_hypotheses[sorted_keys[i-1]]['other_player_next_inventory'] = deepcopy(other_player_next_inventory)
                    counter += 1

                # generate the next high level strategy/inventory
                ...
                while not correct_syntax and counter < 6:
                    ...
                    response = responses[0][0]                        
                    my_next_inventory = self.extract_dict(response)  
                    self.my_next_inventory = deepcopy(my_next_inventory)  
                    hls_response = hls_response + '\n\n' + response   
                    counter += 1
                ...
                response = responses[0][0]                
                self.other_player_next_inventory = self.extract_dict(response)
                self.opponent_hypotheses[best_key]['other_player_next_inventory'] = self.other_player_next_inventory
                # add response to hls after two new lines
                hls_response = hls_response + '\n\n' + response
            ...
            self.my_next_inventory = hls_response
            subgoal_user_msg = self.generate_feedback_user_message(state, execution_outcomes, get_action_from_response_errors, reward_tracker, step)
            responses = await asyncio.gather(
                *[self.controller.async_batch_prompt(self.system_message, [subgoal_user_msg])]
            )
    ...
    def eval_hypotheses(self):
        ...
        for key in keys2eval:
            # Access and use the key and its corresponding 'value'
            if 'predicted_other_player_next_inventory' not in self.opponent_hypotheses[key]['other_player_next_inventory']:
                # make it a tie
                self.opponent_hypotheses[key]['other_player_next_inventory']['predicted_other_player_next_inventory'] = {'cooperate/green': 3, 'defect/red': 3}
            predicted_opponent_next_inventory = self.opponent_hypotheses[key]['other_player_next_inventory']['predicted_other_player_next_inventory']
            empirical_opp_inventory = self.interaction_history[-1]['possible_other_player_inventory']
