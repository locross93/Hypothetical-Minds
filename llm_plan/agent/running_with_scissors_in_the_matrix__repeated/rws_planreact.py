...
class DecentralizedAgent(abc.ABC):
    ...
    def generate_feedback_user_message(
        ...
        strategy_request = f"""
            ...
            You have decided to execute a high-level strategy/target inventory in a previous response.
            ...
            Your response should be broken up into two parts:
                1. Subgoal Plan - based on the current state and the interaction history with the opponent, come up with a strategy. 
            ...
            Example response 1:
            Subgoal Plan: Given the current state and interaction history, I want to counter various players playing paper strategies by collecting more blue resources.
            I would like to get to an inventory of 1 yellow, 1 purple, and 5 blue, and my inventory of 1 yellow, 1 purple, and 3 blue resources,
    ...
    def generate_interaction_feedback_user_message(
        ...
        user_message = f"""
            ...
            1. Think step by step about your opponent's strategy and come up with a strategy for yourself.
            2. 'My_next_inventory': Given your strategy, what should your next inventory be to counter this?
            ...
            Example response 1:
            1. 'My_next_inventory': Given that my opponent is playing a rock policy, I believe my next inventory should be a paper policy inventory={{'rock/yellow': 1, 'paper/purple': 5, 'scissors/blue': 1}}.
            ```python
            {{
              'my_next_inventory': {{'rock/yellow': 1, 'paper/purple': 5, 'scissors/blue': 1}}
            }}
            Example response 2:
            1. 'My_next_inventory': Given that my opponent is playing a paper policy, I believe my next inventory should be a scissors inventory={{'rock/yellow': 1, 'paper/purple': 1, 'scissors/blue': 5}}.
            ```python
            {{
              'my_next_inventory': {{'rock/yellow': 1, 'paper/purple': 1, 'scissors/blue': 5}}
            }}
            Example response 3:
            1. 'My_next_inventory': Given that my opponent is playing a scissors policy, I believe my next inventory should be a rock inventory={{'rock/yellow': 5, 'paper/purple': 1, 'scissors/blue': 1}}.
            ```python
            {{
    ...
    async def two_level_plan(self, state, execution_outcomes, get_action_from_response_errors, reward_tracker, step, after_interaction=False):
        if after_interaction:
            hls_user_msg = self.generate_interaction_feedback_user_message(step) 
            ...
            while not correct_syntax and counter < 6:
                ...
                if 'my_next_inventory' not in self.my_next_inventory:
                    correct_syntax = False  
                hls_response = response   
                counter += 1

            subgoal_user_msg, subgoal_response, goal_and_plan = await self.subgoal_module(state, execution_outcomes, get_action_from_response_errors, reward_tracker, step, after_interaction=True)
        else:
            ...
            self.my_next_inventory = deepcopy(hls_response)
            subgoal_user_msg, subgoal_response, goal_and_plan = await self.subgoal_module(state, execution_outcomes, get_action_from_response_errors, reward_tracker, step, after_interaction=False)
    ...
    async def subgoal_module(self, state, execution_outcomes, get_action_from_response_errors, reward_tracker, step, after_interaction=False):
        user_message = self.generate_feedback_user_message(state, execution_outcomes, get_action_from_response_errors, reward_tracker, step, after_interaction)
        responses = await asyncio.gather(
        ...
        while not valid_plan and counter < 10:
            ...
            user_message = self.generate_feedback_user_message(state, plan_response, get_action_from_response_errors, reward_tracker, step, after_interaction)
