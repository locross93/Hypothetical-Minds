...
class DecentralizedAgent(abc.ABC):
    ...
    def generate_feedback_user_message(
        ...
        strategy_request = f"""
            ...
            Once you've picked up about 5-7 resources in total, seek out a duel to receive rewards, get more information about strategies, and reset your inventory.
    ...
    def generate_interaction_feedback_user_message(
        ...
        user_message = f"""
            ...
            1. 'Opponents_to_seekout':Think step by step about your opponent's strategy and come up with a strategy for yourself, which players should you seek out to duel with next and why? 
            If possible, select opponents you can exploit and maximize your reward. Try to select multiple players if possible as one player might be hard to find or respawning.
    ...
    async def two_level_plan(self, state, execution_outcomes, get_action_from_response_errors, reward_tracker, step, after_interaction=False):
        if after_interaction:
            ...
            while not correct_syntax and counter < 6:
                ...
                if 'my_next_inventory' not in self.hls_next_inventories:
                    correct_syntax = False  
