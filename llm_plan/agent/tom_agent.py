...
class DecentralizedAgent(abc.ABC):
    ...
    def generate_feedback_user_message(
        ...
        strategy_request = f"""
            ...
            Your response should be broken up into two parts:
                ...
                2. Action Plan - output this sequence of actions in the following following Python dictionary format, parsable by `ast.literal_eval()` starting with ```python.
            Example response:
    ...
    def generate_interaction_feedback_user_message(
        ...
        strategy_request = f"""
            ...
            This response should include step by step reasoning about which strategy to select based on the entire interaction history in the following format:
            1. 'Opponent_inventory': What was my opponent's likely inventory in the last round given the inventory I played and the reward received and given the reward function in the system message.
            2. 'Opponent_policy': What is my opponent's likely policy given their inventory and the reward function in the system message. Think step by step about this given the interaction history.
            They may be playing the same pure policy every time, a complex strategy to counter you, or anything in between. They are not necessarily a smart agent that adapts to your strategy.
            Are you getting high positive or negative reward when playing the same type of inventory? For example getting high positive reward every time you play many paper resources. If so, your opponent may be playing a pure strategy and you can exploit this by playing the counter strategy.
            3. 'Opponent_next_inventory': Given my opponent's likely policy, what is their likely inventory in the next round.
            4. 'My_next_inventory': Given my opponent's likely inventory in the next round and the strategy you want to play, what should be my next inventory.
            Use the known information to try and predict your opponent's high-level strategy and select a strategy that counters it. Think step by step for each point 1-4.
            Example response:
            1. 'Opponent_inventory': Given that I last played a pure scissors strategy inventory={{'rock/yellow': 1, 'paper/purple': 1, 'scissors/blue': 5}} and received a large magnitude negative reward, I believe my opponent is playing a rock strategy, potentially the inventory inventory={{'rock/yellow': 5, 'paper/purple': 1, 'scissors/blue': 1}}.
            2. 'Opponent_policy': Given that my opponent is playing a rock policy, I believe their policy is to collect 5 yellow resources and then fire at me.
            3. 'Opponent_next_inventory': Given that my opponent is playing a rock policy, I believe their next inventory will be inventory={{'rock/yellow': 5, 'paper/purple': 1, 'scissors/blue': 1}}.
            4. 'My_next_inventory': Given that my opponent is playing a rock policy, I believe my next inventory should be a paper policy inventory={{'rock/yellow': 1, 'paper/purple': 5, 'scissors/blue': 1}}.
