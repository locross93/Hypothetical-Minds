...
class DecentralizedAgent(abc.ABC):
    ...
    def generate_interaction_feedback_user_message3(
        ...
        user_message = f"""
            An interaction with the other player has occurred at step {step}, {self.interaction_history[-1]}.
            You previously guessed that their policy or strategy is: {self.possible_opponent_strategy}.
            ...
            1. 'Opponent_next_inventory': Given the above mentioned guess about my opponent's policy/strategy, and the last inventory you played (if their strategy is adaptive, it may not be), what is their likely inventory in the next round.
            2. 'My_next_inventory': Given my opponent's likely inventory in the next round, what should my next inventory be to counter this?
    ...
    async def two_level_plan(self, state, execution_outcomes, get_action_from_response_errors, reward_tracker, interaction_rewards, step, after_interaction=False):
        if after_interaction:
            ...
            response = responses[0][0]
            self.possible_opponent_strategy = self.extract_dict(response)
            # add response to hls after two new lines
            hls_response = hls_response + '\n\n' + response
