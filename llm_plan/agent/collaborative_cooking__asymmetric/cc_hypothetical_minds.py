...
class DecentralizedAgent(abc.ABC):
    ...
    def generate_hls_user_message(self, state, step):
        ...
        strategy_request = f"""This response will be shown to you in the future in order for you to select lower-level actions to implement this strategy.
        Example response:
        ...
        """
        user_message = prefix + strategy_request
        ...
        if not self.good_hypothesis_found:
            ...
            while not correct_syntax and counter < 10:
                ...
                for i in range(len(responses)):
                    ...
                    next_behavior = self.extract_dict(response)
    ...
    def update_memory(self, state, step, action):
        ...
        # Place action in the last observed state to check if we changed the state
        if step > 0:
