...
class DecentralizedAgent(abc.ABC):
    ...
    def generate_system_message(self):
        self.system_message = f"""
            ...
            The nuance of 'prisoner_dilemma_in_the_matrix__arena' is that its multi-agent, and that the rewards are using the payoff rules of the classic prisoner dilemma, but the interaction is repeated and the rewards are distributed based on the inventory of resources collected by each player.
    ...
    def generate_feedback_user_message(
        ...
        strategy_request = f"""
            Strategy Request:
            You are at step {step} of the game.
            You have decided to execute a high-level strategy/target inventory in a previous response given what you predicted your given opponent will do.
            Select subgoals in order to achieve the target inventory, denoted by 'my_next_inventory' in this dictionary: {self.hls_next_inventories}.
            Once you achieve the target inventory, seek out a duel with an opponent close to you that you can exploit based on your hypothesis about their strategy and your current inventory.
            Also once you've picked up about 5-7 resources in total, seek out a duel to receive rewards, get more information about strategies, and reset your inventory.
    ...
    def generate_interaction_feedback_user_message1(
        ...
        user_message = f"""
            An interaction with the other player has occurred at step {step}, {self.interaction_history[self.last_played_id][-1]}.
    ...
    def generate_interaction_feedback_user_message3(
        ...
        user_message = f"""
            ...
            Example response 1:
            'Opponent_next_inventory': Given that my opponent is playing a cooperate policy, I believe their next inventory will be inventory={{'cooperate': 5, 'defect': 1}}.
            ```python
            {{
              'predicted_opponent_next_inventory': {{'cooperate': 5, 'defect': 1}}
            }}
            ```
            Example response 2:
            'Opponent_next_inventory': Since my guess is that this player is playing a defect policy, I predict that their next inventory will be {{'cooperate': 1, 'defect': 5}}.
            ```python
            {{
              'predicted_opponent_next_inventory': {{'cooperate': 1, 'defect': 5}}
            }}
            ```
            Example response 3:
            'Opponent_next_inventory': Since my opponent is following a mixed strategy favoring cooperation, I predict their upcoming inventory will be inventory={{'cooperate': 3, 'defect': 2}}
            ```python
            {{
              'predicted_opponent_next_inventory': {{'cooperate': 3, 'defect': 2}}
            }}
            """
        return user_message
    ...
    def generate_interaction_feedback_user_message4(
        ...
        user_message = f"""
            ...
            Example response 1:
            1. 'Opponent_to_seekout': Given that I am fairly certain that player_1 and player_5 is playing a cooperate policy, I believe I should seek out either player_1 or player_5 to duel with next.
            2. 'Opponent_next_inventory': Given that these opponents are playing a cooperate policy, I believe their next inventory will be something like inventory={{'cooperate': 5, 'defect': 1}}.
            3. 'My_next_inventory': Given that these players are playing a cooperate policy, I believe my next inventory should be a defect policy inventory={{'cooperate': 1, 'defect': 5}}.
            ```python
            {{
              'opponents_to_seekout': ['player_1', 'player_5'],
              'predicted_opponent_next_inventory': {{'cooperate': 5, 'defect': 1}}
              'my_next_inventory': {{'cooperate': 1, 'defect': 5}}
            }}

            Example response 2:
            1. 'Opponents_to_seekout': Considering all the interactions, player_2, player_3, and player_5 seem to heavily favor the cooperate strategy with consistent picks. Engaging either of these players could offer a high reward opportunity.
            2. 'Opponent_next_inventory': Based on the observed behavior of player_2, player_3, and player_5, it is likely they will continue with a strong cooperate strategy, potentially having an inventory of {{'cooperate': 5, 'defect': 1}}.
            3. 'My_next_inventory': To effectively counter the cooperate strategy, my optimal approach would be to adopt a defect strategy. An inventory geared towards this would be {{'cooperate': 1, 'defect': 5}}.
            ```python
            {{
            ...
            Example response 3:
            1. 'Opponents_to_seekout': It is clear from all the interactions that every player is playing cooperate. I should seek out any player to duel with next.
            2. 'Opponent_next_inventory': Assuming the players continue this strategy, their next inventory is likely to be {{'cooperate': 5, 'defect': 1}}.
            3. 'My_next_inventory': To counter the population's rock/yellow focus, I should heavily collect paper/purple resources. My anticipated inventory would thus be {{'cooperate': 1, 'defect': 5}}.
            ```python
            {{
    ...
    async def two_level_plan(self, state, execution_outcomes, get_action_from_response_errors, reward_tracker, step, after_interaction=False): 
        if after_interaction:
            ...
            if not good_hypothesis_found:
                ...
                self.opponent_hypotheses[self.last_played_id][self.interaction_num]['value'] = 0
                ...
                while not correct_syntax and counter < 15:
                    ...
                    response, hls_next_inventories = self.parse_multiple_llm_responses(response, response_type='next_inventories')
