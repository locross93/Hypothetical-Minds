...
class DecentralizedAgent(abc.ABC):
    ...
    def generate_system_message(self):
        self.system_message = f"""
            ...
            Your reward is the result of a matrix multiplication involving the your inventory in a vector format, and your opponent's inventory vector, and a payoff matrix similar to prisoner's dilemma.
            r_t = transpose(your_inventory) * A_payoff * opponent_inventory
            where A_payoff = np.array([[3, 0], [5, 1]])
            ...
            To play a strategy strongly, pick up at least 5 resources or more of only one color and then fire the interaction beam at the other player.
            To commit less strongly to a strategy, pick up around 3 resources of only color and then fire the interaction beam at the other player.
    ...
    def generate_interaction_feedback_user_message1(
        ...
        user_message = f"""
            ...
            and then think about which resource your opponent must have had the most of in their inventory given the reward you received.
            ...
            A possible inventory for them could be {{'cooperate/green': 1, 'defect/red': 6}} or a similar balance favoring defect.
            ```python
            {{
                ...
                while not correct_syntax and counter < 6:
                    ...
                    for i in range(len(responses)):
                        ...
                        if not both_keys_present:
                            ...
                            break
                        if i == 0:
                            self.next_inventories = next_inventories
                            self.opponent_hypotheses[self.interaction_num]['next_inventories'] = self.next_inventories
                        else:
                            self.opponent_hypotheses[sorted_keys[i-1]]['next_inventories'] = next_inventories
                    counter += 1
                # add response to hls after two new lines
                hls_response = hls_response + '\n\n' + response
