...
"""A custom Gym space for byte string."""
class ByteStrSpace(spaces.Space):
    """A Gym space for arbitrary objects."""
    def __init__(self):
        super().__init__(shape=(), dtype=np.bytes_)

    def sample(self):
        raise NotImplementedError("ByteStrSpace cannot be sampled randomly.")

    def contains(self, x):
        return isinstance(x, bytes)  # Adjust based on what types you expect
