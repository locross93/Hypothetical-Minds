...
async def run_main(agent_type, substrate, llm_type, num_seeds, scenarios):
    ...
    if substrate == 'collaborative_cooking__asymmetric':
        df_plot = pd.read_csv('results/cc_scores.csv')
    elif substrate == 'running_with_scissors_in_the_matrix__repeated':
        df_plot = pd.read_csv('results/rws_scores.csv')
    elif substrate == 'prisoners_dilemma_in_the_matrix__repeated':
        df_plot = pd.read_csv('results/pd_scores.csv')
    elif substrate == 'running_with_scissors_in_the_matrix__arena':
        df_plot = pd.read_csv('results/rws_arena_scores.csv')
    ...
    for scenario_num in scenarios:
        ...
        agent_label = agent_type+'_'+llm_type
        filtered_df = df_plot[(df_plot['agent_type'] == agent_label) & (df_plot['scenario'] == scenario_num)]
...
async def main():
    ...
    parser.add_argument("--substrate", type=str, default="rws", help="Substrate type")
    parser.add_argument("--llm_type", type=str, default="llama3", help="LLM type (default: gpt4)")
