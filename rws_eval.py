...
from llm_plan.agent.hierarchical_agent import DecentralizedAgent
...
async def main_async(scenario_num: int):
    ...
    agent = setup_agent(api_key, model_id=f"player_0", model_settings=model_settings)
    ...
    frame_folder = f'./frames/{substrate_name}/agent_v1/scenario_{scenario_num}_{date_time_str}'
    ...
    user_messages = [agent.generate_initial_user_message(state, step)]        
    ...
    with open(output_data_path, 'a') as file:
        file.write(f"Hierarchical Agent, playing scenario {scenario_num}\n\n")
        file.write(f"Hierarchical Agent, playing scenario {scenario_num}\n\n")
        file.write(f"Initial response: {response}\n\n")
    ...
    while not valid_plan and counter < 4:
        ...
        responses[0] = await asyncio.gather(
            *[agent.controller.async_batch_prompt(system_messages[0], [plan_response])]
        )
    ...
    #while step < 200:
        ...
        if len(agents_no_actions) > 0 and not during_interaction:
            ...
            while not valid_plan and counter < 4:
                ...
                response = responses[0][0][0]
                goal_and_plan = agent.extract_goals_and_actions(response)
                if len(goal_and_plan) == 0 or len(goal_and_plan['action_plan']) == 0 or 'action_plan' not in goal_and_plan:
        ...
        if after_interaction and reset_env:
            ...
            during_respawn = False
            interaction_num += 1
            interaction_history.append(f"Interaction {interaction_num}: your_inventory={interaction_inventory} rewards={interaction_rewards[agent.agent_id].item()}")
            # give feedback and reset all plans
            # get prompts for each agent
            system_messages = [agent.system_message for agent in agents]
            user_messages = [
                    agent.generate_interaction_feedback_user_message(state, execution_outcomes, get_action_from_response_errors, reward_tracker, interaction_rewards, interaction_history, step) 
                    for agent in agents]
            user_message_data_list.append(user_messages)
            system_messages_data_list.append(system_messages)
            # Gathering responses asynchronously
            ...
            while not valid_plan and counter < 4:
                print(f"Invalid plan for {agent.agent_id}. Trying again.")
                user_message = agent.generate_interaction_feedback_user_message(state, execution_outcomes, get_action_from_response_errors, reward_tracker, interaction_rewards, interaction_history, step)
                plan_response = plan_response + user_message
                responses[0] = await asyncio.gather(
                    *[agent.controller.async_batch_prompt(system_messages[0], [plan_response])]
                )
                ...
                if len(goal_and_plan) == 0 or len(goal_and_plan['action_plan']) == 0 or 'action_plan' not in goal_and_plan:
                    ...
                    valid_plan, plan_response = agent.is_valid_plan(state, goal_and_plan)
                counter += 1
            print(f"Interaction {interaction_num} response: {response}")
            print(agent.agent_id, goal_and_plan)
            with open(output_data_path, 'a') as file:
                file.write(f"Interaction {interaction_num}, inventory={interaction_inventory}, rewards={interaction_rewards[agent.agent_id].item()}\n")
                file.write(f"{interaction_history}\n")
    ...
    df_results = pd.DataFrame({'agent_type': ['v1'], 'scenario': [scenario_num], 
