...
async def main_async():
    ...
    # substrate_name = 'running_with_scissors_in_the_matrix__repeated_8' 
    ...
    while step < 200:
        ...
        for agent in agents:
            ...
            if action and action[:8] == 'INTERACT' or action == 'FIRE_ZAP':
                ...
                if not interaction and agent.all_actions.qsize() == 0 and agent.interact_steps < 20:
        ...
        for agent_id, reward in rewards.items():
            ...
            if reward != 0:
                #player_inventory = state[agent_id]['inventory']
                #print(f"Interaction: Agent {agent_id}, inventory: {player_inventory}, received reward {reward}")
        ...
        save_obs_frames(obs, step, frame_folder)
        print(f"Step {step} rewards: {step_rewards}, Player 0 inventory: {state['player_0']['inventory']}, Player 1 inventory: {state['player_1']['inventory']}")
        if interaction:
