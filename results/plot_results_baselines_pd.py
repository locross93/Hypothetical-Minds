...
df_plot = pd.read_csv('pd_scores.csv')

#df_plot = pd.read_csv('pd_heuristics.csv')
...
scenarios = range(10)  # scenarios 0 to 8
...
# add rl results
df_rl = pd.read_csv(f'prisoners_dilemma_in_the_matrix__repeated/eval_results_all_scenarios.csv')
...
scenario_grouped_df.to_csv('pd_scenario_grouped_df.csv', index=False)
...
sns.despine(top=True, right=True)
plt.title('Prisoners Dilemma Repeated', fontsize=28, pad=25)
plt.ylabel('Reward', fontsize=28)
